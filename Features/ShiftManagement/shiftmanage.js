  ...
  async handleInteraction(interaction) {
    try {
      ...
      if (action === 'end') {
        ...
        // allowed to end if onShift or onBreak
        const endTime = Date.now();
        let breaks = shift.breaks || [];
        // if onBreak, close current break
        if (shift.status === 'onBreak' && shift.currentBreakStart) {
          breaks = breaks.concat([{ start: shift.currentBreakStart, end: endTime }]);
        }

        const totalBreakMs = breaks.reduce((s,b) => s + (b.end - b.start), 0);
        const totalMs = Math.max(0, endTime - shift.startTime - totalBreakMs);

        // update user stats: increment count and add totalMs
        const updatedStats = updateUserStats(shift.userId, shift.typeId, totalMs);
        const count = updatedStats.count || 0;
        const totalTime = updatedStats.totalTimeMs || 0;
        const avg = count > 0 ? Math.round(totalTime / count) : 0;
        ...
        // remove the active shift entry
        removeShiftObj(shiftId);

        await interaction.update({ embeds: [embedAll, embedLast], components: [row] });
        ...
        if (cfg && cfg.SHIFT_LOG_CHANNEL) {
          ...
          if (ch) {
            const log = new EmbedBuilder()
              ...
              .addFields(
                { name: 'User', value: `<@${interaction.user.id}>` },
                { name: 'Type', value: shift.typeName },
                { name: 'Total Time', value: `${formatDuration(totalMs)}` },
                { name: 'At', value: `<t:${Math.floor(endTime/1000)}:F>` }
              );
            ch.send({ embeds: [log] }).catch(()=>{});
          }
        }
